#!/usr/bin/env python3
"""Run data quality checks on the cleaned Helsinki City Bikes dataset."""
...
DEFAULT_INPUT = PROJECT_ROOT / "data" / "processed" / "2021-04_cleaned.csv"
DEFAULT_SAMPLE_DIR = PROJECT_ROOT / "data" / "derived" / "data_quality_samples"
...
if len(very_short) > 0:
    ...
    very_short.head(20).to_csv(args.sample_dir / 'sample_short_trips.csv', index=False)

if len(very_long) > 0:
    print("Saving sample of very long trips...")
    very_long.head(20).to_csv(args.sample_dir / 'sample_long_trips.csv', index=False)

if len(unrealistic_speed) > 0:
    print("Saving sample of unrealistic speed trips...")
    unrealistic_speed.head(20).to_csv(args.sample_dir / 'sample_fast_trips.csv', index=False)
